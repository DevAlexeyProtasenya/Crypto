...
function checkAmount() {
    ...
    var amountReg = /[\d]+[.[0-9]+]?/;
    if(amountReg.test(inputAmount.val())!==true){
        amountReg.css('border-color','red');
        return false;
    }
    return true;
}
...
function checkCard() {
    var inputCardNumber= $("input[name='creditCard']");
    ...
    var inputSvcCode= $("input[name='cvvOnCard']");
    if(inputSvcCode.val().length===0){
        inputSvcCode.css('border-color','red');
        alert("CVV номер введены неверно!");
        return false;
    }
    var svcReg = /^[0-9]{3,4}$/;
    if(svcReg.test(inputSvcCode.val())!==true){
        inputSvcCode.css('border-color','red');
        alert("CVV номер введен неверно!");
        return false;
    }
    return true;
}
