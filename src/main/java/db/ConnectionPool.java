...
public class ConnectionPool {
    ...
    private static final DBConfiguration DB_CONFIG = new DBConfiguration();

    private ConnectionPool() {
        connectionQueue = new ArrayBlockingQueue<>(DB_CONFIG.getPool());
        try {
            DriverManager.registerDriver(new com.mysql.jdbc.Driver());
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
        for (int i = 0; i < DB_CONFIG.getPool(); i++) {
            addConnection();
        }
    }
